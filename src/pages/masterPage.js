...
$w.onReady(function () {
    ...
    // 5. FAQPage schema (only if the page has visible FAQ content)
    // This is a safe basic version. You can expand it later with real Q&A pairs.
    const faqElements = $w('Text').filter((el) => {
        const txt = (el.text || '').toLowerCase();
        return txt.includes('faq') || txt.includes('frequently asked') || txt.startsWith('q.');
    });

    if (faqElements.length > 0 && path.length > 0 && path[0].startsWith('artificial-grass-')) {
        // Basic FAQPage placeholder – replace with real questions later for best results
        schemas.push({
            "@context": "https://schema.org",
            "@type": "FAQPage",
            "mainEntity": [
